	...
	public static boolean download(String url, File savePath, IProgress call) {
		...
		try(Auto conn = new Auto(new URL(url).openConnection()).connect();
			...
			while((read = net.read(buf)) != -1) {
				...
				double progress = Math.ceil((double)currentLength / conn.connection.getContentLengthLong()*100000) / 100000 * 100;
				progress = Double.parseDouble(String.format("%.3f", progress));
