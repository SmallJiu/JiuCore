...
public class HighlightBlock extends RenderType {
    ...
    public void render(Highlight light, PoseStack stack, Matrix4f matrix4f, Camera camera) {
        ...
        if (camera.isInitialized()) {
            ...
            BufferBuilder buffer = Tesselator.getInstance().begin(CUBE_RENDER.mode(), CUBE_RENDER.format());
            stack.pushPose();
            ...
            this.vertex.bind();
            this.vertex.upload(buffer.buildOrThrow());
            VertexBuffer.unbind();
    ...
    public void drawSide(Vec3 tr, Vec3 tl, Vec3 br, Vec3 bl, ColorData color, VertexConsumer buf, PoseStack stack) {
        ...
        buf.addVertex(matrix4f, (float) tr.x, (float) tr.y, (float) tr.z).setColor(color.getRf(), color.getGf(), color.getBf(), color.getAf());
        buf.addVertex(matrix4f, (float) br.x, (float) br.y, (float) br.z).setColor(color.getRf(), color.getGf(), color.getBf(), color.getAf());
        buf.addVertex(matrix4f, (float) bl.x, (float) bl.y, (float) bl.z).setColor(color.getRf(), color.getGf(), color.getBf(), color.getAf());
        buf.addVertex(matrix4f, (float) tl.x, (float) tl.y, (float) tl.z).setColor(color.getRf(), color.getGf(), color.getBf(), color.getAf());
    }
