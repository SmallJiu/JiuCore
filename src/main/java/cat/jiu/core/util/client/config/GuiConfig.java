...
public class GuiConfig extends Screen {
    ...
    public GuiConfig(Minecraft mc, Screen parent) {
        this(parent);
        this.minecraft = mc;
    }
    ...
    AtomicReference<SoundMC> sound = new AtomicReference<>();
    @Override
    protected void init() {
        if (false){
            this.addRenderableWidget(Button.builder(Component.literal("Test"), button -> {
                if (sound.get() != null) {
                    ...
                    sound.set(new SoundMC(SoundEvents.MUSIC_DISC_CAT));
                    sound.get().play();
                }
            }).pos(0, 50).size(50, 20).build());
        }
        ...
        if(this.modid == null && this.canEdit && this.path != null && !this.path.startsWith(":server")) {
...
//            return super.mouseClicked(pMouseX, pMouseY, pButton);
        ...
        public static class ConfigListEntry extends ObjectSelectionList.Entry<ConfigListEntry> {
            ...
            public boolean mouseReleased(double mouseX, double mouseY, int button) {
                return this.entry.mouseReleased(mouseX, mouseY, button);
            }

            @Override
            public void mouseMoved(double mouseX, double mouseY) {
                this.entry.mouseMoved(mouseX, mouseY);
            }

            @Override
            public boolean mouseDragged(double mouseX, double mouseY, int button, double dragX, double dragY) {
                return this.entry.mouseDragged(mouseX, mouseY, button, dragX, dragY);
            }
            ...
            public boolean keyReleased(int keyCode, int scanCode, int modifiers) {
                return this.entry.keyReleased(keyCode, scanCode, modifiers);
            }
