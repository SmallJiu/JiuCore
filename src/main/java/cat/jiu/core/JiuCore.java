...
import cat.jiu.core.api.events.GameShutdownEvent;
import cat.jiu.core.api.events.game.IGameShutdown;
...
public class JiuCore implements IInFluidCraftingEvent {
	protected static Logger logger = LogManager.getLogger(JiuCore.MODID);
	protected static LogOS logos = new LogOS();
	...
	public static final String VERSION = "1.1.0-20220608013004";

	private static Boolean isTest = null; // if is IDE, you can set to '1' to enable some test stuff
	public static final boolean test() {
		if(isTest == null) {
			isTest = new File("./config/jiu/core_debug.jiu").exists();
		}
		return isTest;
	}
	...
	public void loadComplete(FMLLoadCompleteEvent event) {
		...
		if(list != null) list.stream().forEach(e -> e.onAddInFluidCrafting(recipe));
	}
	...
	public static Logger getLogger() {
		if(logger == null) {
			logger = LogManager.getLogger(JiuCore.MODID);;
		}
		return logger;
	}
